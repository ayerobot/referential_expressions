...
def generate_feature_vectors(data, commands, world):
	...
	f_data_all = []
	for i in range(1, 13):
		f_matrix_all.append(get_feature_matrix(x, y, commands[i], world))
		f_data_all.append(get_feature_matrix(data[i][:,0], data[i][:,1], commands[i], world))
	Tx = np.vstack(f_matrix_all)
	theta = np.array(np.mean(np.vstack(f_data_all), 0))*np.array([0.57, 0.19, 0.24])

	sio.savemat('data/test.mat', {'Tx' : Tx, 'theta' : theta})
	return Tx, theta
